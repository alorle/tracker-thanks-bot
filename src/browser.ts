...
export type BrowserContexts = {
  ...
  freshPage: (siteKey: string) => Promise<Page>;
  closeAll: () => Promise<void>;
};

export function createBrowserContexts(cacheDir: string): BrowserContexts {
  const contexts = new Map<string, BrowserContext>();

  async function getContext(siteKey: string): Promise<BrowserContext> {
   ...
   */
  async function freshPage(siteKey: string): Promise<Page> {
  ...
  return { getContext, freshPage, closeAll };
}
